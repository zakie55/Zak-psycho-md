...
async function startBot() {
    ...
    sock.ev.on('connection.update', async (update) => {
        ...
        if (connection === 'open') {
            ...
            if (owner) {
                await sock.sendMessage(owner + '@s.whatsapp.net', { text: 'Welcome to Zak Psycho MD Bot! 🤖🔥' });
            }
        }
        if (connection === 'close') {
    ...
    sock.ev.on('messages.upsert', async (m) => {
        ...
        if (messageText.startsWith('.menu')) {
            let menuText = `*📜 Zak Psycho MD Bot Commands 📜*\n\n
...
async function handleUserInput() {
    ...
    if (startCommand.toLowerCase() === 'startbot') {
        ...
        console.log(chalk.red('\nInvalid command! Type "startbot" to run.'));
    }
}
